...
| ALR-017 | P2 | 実地点の日の出・日の入りを任意反映する | 固定時刻の空が地域・季節とずれる |
...
## ALR-017: 実地点の日の出・日の入りを任意反映する

**Priority:** P2

### 背景

現行の背景は位置情報権限なしで即時利用できるよう、端末local timeの06:00 / 18:00を日の出・日の入りの基準にしている。実際の太陽状態は緯度・経度・日付で変化し、国籍からは決定できない。

### スコープ

- 「端末の位置情報を使う」「地域を手動選択」「基準時刻のまま」の選択肢を設計する
- opt-in時だけ緯度・経度を取得し、端末内で日の出・日の入りを算出する
- precise locationを不要にできるか評価し、保存期間と削除方法を明示する
- 権限拒否、timezone変更、極昼・極夜でも固定基準へ安全にfallbackする
